...
class MenuItemCounter extends StatefulWidget {
  ...
  MenuItemCounter(
      ...
      this.foodRecords,
      this.parent})
      : super(key: menuItemCounterKey);

  @override
  MenuItemCounterState createState() {
    return new MenuItemCounterState(
        priceFieldKey, this.foodRecords, this.parent, this.menuItemCounterKey);
  }
}
...
class MenuItemCounterState extends State<MenuItemCounter> {
  ...
  MenuItemCounterState(this.priceFieldKey, this.foodRecords, this.parent,
      this.menuItemCounterKey);
              ...
              itemBuilder: (BuildContext context, int index) {
                ...
                return Dismissible(
                  ...
                  onDismissed: (direction) async {
                    AmplitudeAnalytics.analytics.logEvent('remove_from_cart ',
                        eventProperties: {'uuid': order.product.uuid});
                    currentUser.cartModel = await deleteItemFromCart(
                        necessaryDataForAuth.device_id, order.id);
                    currentUser.cartModel.items.remove(order);
                    Navigator.pop(context);
                    parent.setState(() {});
                    if (parent.parent.basketButtonStateKey.currentState !=
                            null &&
                        currentUser.cartModel.items.length == 0) {
                      parent.parent.basketButtonStateKey.currentState
                          .setState(() {});
                    }
                  },
        ...
        children: [
          Align(
            ...
            child: ClipRRect(
              ...
              child: Image.network(
                getImage((order.product.meta.images != null &&
                        order.product.meta.images.length != 0)
                    ? order.product.meta.images[0]
                    : ''),
                fit: BoxFit.cover,
                height: 70,
                width: 70,
            ...
            child: Column(
              children: [
                ...
                (order.variantGroups != null)
                    ? Container(
                        child: Column(
                          children: List.generate(order.variantGroups.length,
                              (index) {
                            return Column(
                              ...
                              return Align(
                                ...
                                child: Padding(
                                  ...
                                  child: Text(
                                    order.variantGroups[index]
                                        .variants[variantsIndex].name,
                                    style: TextStyle(
  ...
  Future<void> _incrementCounter_plus() async {
    currentUser.cartModel =
        await changeItemCountInCart(necessaryDataForAuth.device_id, item.id, 1);
    setState(() {});
  }
  ...
  Future<void> _incrementCounter_minus({bool ignoreVariable = false}) async {
    if (item.product.type == 'variable' && !ignoreVariable) {
      ...
      currentUser.cartModel.items.forEach((element) {
        if (element.product.uuid == item.product.uuid) count++;
      });
      ...
      currentUser.cartModel = await changeItemCountInCart(
          necessaryDataForAuth.device_id, item.id, -1);
      item = currentUser.cartModel.findCartItem(foodRecords);
      if (item.count == 0 || item == null) {
        currentUser.cartModel =
            await deleteItemFromCart(necessaryDataForAuth.device_id, item.id);
        parent.setState(() {});
  ...
  List<Item> findCartItems(ProductsByStoreUuid product) {
    ...
    try {
      items.addAll(currentUser.cartModel.items
          .where((element) => element.product.uuid == product.uuid));
    } catch (e) {
      items = new List<Item>();
    }
    return items;
          ...
          Container(
            ...
            child: Padding(
              ...
              child: Center(
                child: Text(
                  '$counter',
                  style: TextStyle(fontSize: 18.0, fontWeight: FontWeight.w400, color: AppColor.textColor),
                ),
              ),
            ),
          ),
